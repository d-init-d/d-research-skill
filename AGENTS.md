...
Core workflow:
0. Classify the request with `references/research-intake.md` before opening sources. Assign multi-label research shape(s), safety posture, expected output artifact, freshness/geography/language scope, required references, required ledgers/templates, and execution gates. Resolve hard-stop safety/privacy/access issues before source discovery; ask the user only when ambiguity changes safety, legality, scope, or deliverable.
