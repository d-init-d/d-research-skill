...
- Eval harness offline để bắt regression khi sửa `SKILL.md` / `AGENTS.md` / `references/*`: bench `examples/evals/dogfood-bench.json` (12 task, 4 class — atomic-fact, api-workflow, contradiction, person-aggregation) + harness stdlib-only `scripts/run_dogfood.py` (subcommand `self-test`, `validate`, `list`, `render`, `score`, `baseline`). CI chạy `eval:self-test` mỗi PR. Để thật sự score 1 ledger thì chạy `npm run eval:score -- DF-001 path/to/ledger.csv`. Đây là regression detector, không phải leaderboard — không ship điểm số per-agent. Xem `docs/eval.md`.
