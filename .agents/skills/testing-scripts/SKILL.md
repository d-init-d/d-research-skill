...
If you want to isolate a failure, run scripts one at a time. There are **21 helper scripts** with self-tests (5 Node + 16 Python). `run_python.mjs` is a thin wrapper and has no self-test of its own.
...
python3 scripts/run_dogfood.py         self-test   # → "OK: eval benches valid; dogfood-bench.json: 12 tasks, frontier-bench.json: 36 tasks."
...
If any of the 21 self-tests fail locally, the same failure will block the PR. Fix locally before pushing.
