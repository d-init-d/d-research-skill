...
def get_stats(input_path: Path) -> dict[str, Any]:
    ...
    headers, rows = read_csv(input_path)
    col_stats = defaultdict(lambda: {'types': set(), 'missing': 0, 'samples': []})
    
    for row in rows:
        for header in headers:
            value = row.get(header, '')
            if value.strip() == '':
                col_stats[header]['missing'] += 1
            col_stats[header]['types'].add(infer_type(value))
            if len(col_stats[header]['samples']) < 3:
                col_stats[header]['samples'].append(value)
    ...
    return {
        ...
        'columns': [
            {
                'name': header,
                'type': max(stats['types'], key=lambda t: list(stats['types']).count(t)) if stats['types'] else 'unknown',
...
def run_self_test() -> bool:
    ...
    try:
        ...
        print("  ✓ clean command passed (4 unique rows)")
        ...
        print("  ✓ validate command detected type mismatch")
        ...
        print("  ✓ merge command passed (left join successful)")
...
def main() -> int:
    ...
    # clean command
    clean_parser = subparsers.add_parser('clean', help='Clean CSV: remove duplicates, trim whitespace, normalize')
    ...
    # stats command
    stats_parser = subparsers.add_parser('stats', help='Print CSV statistics as formatted table')
    stats_parser.add_argument('--file', required=True, type=Path, help='Input CSV file')
    
    # dedup command
    dedup_parser = subparsers.add_parser('dedup', help='Remove duplicates by key field (keep first)')
    dedup_parser.add_argument('--file', required=True, type=Path, help='Input CSV file')
    dedup_parser.add_argument('--key', required=True, help='Key field name for deduplication')
    dedup_parser.add_argument('--out', required=True, type=Path, help='Output CSV file')
    
    # validate command
    validate_parser = subparsers.add_parser('validate', help='Validate CSV against JSON schema')
    validate_parser.add_argument('--file', required=True, type=Path, help='Input CSV file')
    validate_parser.add_argument('--schema', required=True, type=Path, help='JSON schema file')
    
    # merge command
    merge_parser = subparsers.add_parser('merge', help='Left join CSVs on key field')
    merge_parser.add_argument('--files', required=True, nargs=2, type=Path, metavar='FILE', help='Input CSV files')
    ...
    try:
        if args.command == 'clean':
            ...
            print(f"✓ Cleaned data written to {args.out}: {count} rows")
            return 0
            ...
            print(f"✓ Deduplicated by '{args.key}' written to {args.out}: {count} rows")
            return 0
            ...
            if errors:
                ...
                print("✓ CSV validates against schema")
                return 0
        
        elif args.command == 'merge':
            count = merge_data(args.files, args.key, args.out)
            print(f"✓ Merged on '{args.key}' written to {args.out}: {count} rows")
            return 0
