...
- `scripts/translate.py`: text / detect / instances / self-test — translation adapter with optional deterministic langdetect, stdlib trigram fallback, and LibreTranslate/DeepL/Google/Argos backends; see `adapters/translation.md`
- `scripts/embed_corpus.py`: index / query / query-ledger / dedupe / self-test — semantic retrieval over text corpora using cosine similarity; auto selects the optional local sentence-transformers backend, while stub/remote/CLI backends require explicit selection; see `references/semantic-retrieval.md`
